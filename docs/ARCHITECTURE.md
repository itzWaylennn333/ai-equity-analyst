...
| **P7 — Predictive** | **Tiers 1–3 done** (deterministic, evidence-verified, adversarially reviewed): honesty harness (`src/backtest.py` — Deflated/Probabilistic Sharpe, **PBO/CSCV**, **purged & embargoed CV**, MinBTL, **\|t\|>3 gate** + Holm/BHY, decay haircut); calibration (`src/calibrate.py` — Platt/isotonic/beta + ECE/Brier + **walk-forward drift monitor**); conformal intervals (`src/conformal.py` — split / vol-scaled + **ACI** + rolling/crisis coverage + Kupiec POF). Remaining: the LightGBM + ElasticNet factor+sentiment **model** — needs a multi-stock point-in-time panel (data-assembly step). | L | 🔄 Tiers 1–3 done |
